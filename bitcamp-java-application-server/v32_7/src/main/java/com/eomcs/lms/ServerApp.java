...
public class ServerApp {
  ...
  private static void updateLesson0() throws Exception {
    ...
    for (Lesson object : lessonList) {
      if (object.getNo() == lesson .getNo()) {
        // 클라이언트가 보낸 값으로 기존 객체의 값을 바꾼다.
        object.setNo(lesson .getNo());
        object.setTitle(lesson .getTitle());
        object.setContents(lesson .getContents());
        object.setStartDate(lesson .getStartDate());
        object.setEndDate(lesson .getEndDate());
        object.setTotalHours(lesson .getTotalHours());
        object.setDayHours(lesson .getDayHours());
  ...
  private static void updateBoard0() throws Exception {
    ...
    for (Board obj : boardList) {
      if (obj.getNo() == board.getNo()) {
        // 클라이언트가 보낸 값으로 기존 객체의 값을 바꾼다.
        obj.setNo(board.getNo());
        obj.setContents(board.getContents());
