...
public class ServerApp {
  ...
  private static void updateMember() throws Exception {
    ...
    /*
     * for (int i = 0; i < memberList.size(); i++) { Member m = memberList.get(i); if (m.getNo() ==
     * member.getNo()) { // 기존 객체를 클라이언트가 보낸로 교체한다. memberList.set(i, member); out.writeUTF("ok");
     * return; } } out.writeUTF("fail"); out.writeUTF("해당 번호의 회원이 없습니다.");
     */
  }
  ...
  private static void deleteMember() throws Exception {
    ...
    /*
     * for (Member m : memberList) { if (m.getNo() == no) { memberList.remove(m);
     * out.writeUTF("ok"); return; } } out.writeUTF("fail"); out.writeUTF("해당 번호의 회원이 없습니다.");
     */
  }
