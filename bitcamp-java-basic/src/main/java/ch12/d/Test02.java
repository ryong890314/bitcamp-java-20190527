...
public class Test02 {
  public static void main(String[] args) {
    ...
    // "추상화"라 부른다.
    // => 즉 실세계의 객체(사람, 물건, 업무 등)를 컴퓨터에서 다룰 수 있도록 클래스로 정의하는 행위를 "추상화"라 부른다. 
    
    Patient2 p2 = new Patient2();
    p2.setName ("이철희");
    p2.setAge (300); // 캡슐화를 무너뜨릴 수 있는 유효하지 않은 값은 무시 된다.
    p2.setWeight (-50); // 캡슐화를 무너뜨릴 수 있는 유효하지 않은 값은 무시  된다.
    p2.setHeight(400); // 캡슐화를 무너뜨릴 수 있는 유효하지 않은 값은 무시  된다.
    p2.setGender(3); // 캡슐화를 무너뜨릴 수 있는 유효하지 않은 값은 무시  된다.
    ...
    // 몸무게가 -50이면 이해 불가하다.
    // 키가 4m이면 나무다.
    // 즉 Patient 클래스는 환자의 데이터를 저장할 목적으로 정한 클래스인데 
