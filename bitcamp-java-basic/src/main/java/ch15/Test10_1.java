...
public class Test10_1 {
  public static void main(String[] args) {
    ...
    // 정수 값을 얻는다.
    // => 그렇게 리턴받은 정수 값(해시 코드)를 사용하여 객체를 저장할 위치를 계산한다.
    ...
    // 결론!
    // => k2와 k6는 다른 객체지만, HashCode()의 리턴 값이 같고, equals()의 리턴 값이 true이기 때문에
    // 두 객체는 같은 key로 간주 된다.
    ...
    // Object 클래스에서 상속 받은 hashCode()는 인스턴스 필드의 값이 같은지 따지지 않고
    // 무조건 인스턴스마다 고유의 해시 값을 리턴한다.
    // 또한 equals()는 인스턴스 필드의 값이 같은지 비교하지 않고 같은 인스턴스인지만 따진다.
    // 
    // 그러나 Integer 클래스와 같은 wrapper 클래스와 String 클래스는 Object 클래스에서 상속 받은
    // hashCode()와 equals()를 오버라이딩 했기 때문에 인스턴스가 다르더라도 인스턴스 필드의 값이 같으면
    // hashCode()의 리턴 값이 같고 equals()가 true를 리턴한다.
    // 그래서 wrapper 클래스와 String 클래스는 HashMap/Hashtable의 key로 사용할 수 있는 것이다.
